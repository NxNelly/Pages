const canva: HTMLCanvasElement = <HTMLCanvasElement>document.getElementById("canvas")!;
const crc: CanvasRenderingContext2D = canva.getContext("2d")!;
...
let trees: {[key: string]: {height:number; width : number; treePosX: number; treePosY: number; leafPosX: number; leafPosY: number} } = {
    tree0 : {height: 200,
        width : 40, 
        treePosX: 150,
        treePosY: 520,
...
function drawLeaves(getleafPosX: number, getleafPosY: number) {
    crc.fillStyle = "green";
...
function drawAnimal(posX: number,posY: number){
    ...
    let pathEyes:Path2D = new Path2D();
...
let PosAnimalX: number =100;
let PosAnimalY: number =550;
